...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete MTC4ei database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no MTC4ei datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old MTC4ei datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected MTC4ei 0.7 datadir."
    ;;
  3)
    echo "Detected MTC4ei pre-0.8 datadir."
    ;;
  4)
    echo "Detected MTC4ei 0.8 datadir."
    ;;
esac

...
*Setup instructions for a Gitian build of MTC4ei Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MTC4ei
...
- [Building MTC4ei Core](#building-mtc4ei-core)
...
Clone the git repositories for mtc4ei and Gitian.
...
in the mtc4ei repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building MTC4ei Core
----------------

To build MTC4ei Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the mtc4ei repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mtc4ei/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/mtc4ei-project/mtc4ei
...
For example:
```bash
URL=https://github.com/thrasher-/mtc4ei.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit mtc4ei=${COMMIT} --url mtc4ei=${URL} ../mtc4ei/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mtc4ei=${COMMIT} --url mtc4ei=${URL} ../mtc4ei/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mtc4ei=${COMMIT} --url mtc4ei=${URL} ../mtc4ei/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the mtc4ei git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../mtc4ei/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/mtc4ei
SIGPATH=/some/root/path/mtc4ei-detached-sigs

./bin/gbuild --url mtc4ei=${BTCPATH},signature=${SIGPATH} ../mtc4ei/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mtc4ei-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mtc4ei-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/mtc4ei-osx-build.assert
```
...
[mtc4ei-project/gitian.sigs.ltc](https://github.com/mtc4ei-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull

MTC4ei Core version 0.13.2 is now available from:

  <https://download.mtc4ei.org/mtc4ei-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a mtc4ei
...
ECDSA signatures inside MTC4ei transactions now use validation using
...
Previous versions of MTC4ei Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of MTC4ei Core was to raise the default minimum
relay fee.

MTC4ei Core 0.13.2 will have a strict maximum size on the mempool. The
...
MTC4ei Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running mtc4eid without having to do any manual configuration.
...
MTC4ei Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. MTC4ei Core will relay transactions with
...
entirely in MTC4ei Core 0.13, and it is at this time undecided whether the
...
MTC4ei Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
MTC4ei Core automatically creates a hidden service to listen on, without
manual configuration. MTC4ei Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if MTC4ei Core is listening, and
...
MTC4eid can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals MTC4ei
Core to use floating fees. By default, MTC4ei Core will use floating
fees.
...
(default: `0.0002` MTC4/kB).

At all times, MTC4ei Core will cap fees at `-maxtxfee=<x>` (default:
0.10) MTC4.
Furthermore, MTC4ei Core will never create transactions paying less than
...
`mtc4ei.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 29336 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 29336 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /mtc4eirpc>
        ...
        ProxyPassReverse http://127.0.0.1:9336/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass mtc4eid rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< mtc4eirpc:<password>
    </Location>
...
Restarting mtc4eid will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in mtc4ei.conf
...
For example:
    ...
    $ src/mtc4ei-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The MTC4ei Core code base has
...
- `mtc4ei-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `mtc4ei-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the MTC4ei scripting system that in combination with
...
MTC4ei transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of MTC4ei Core.
...
The two phases are distinct in the MTC4ei-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming MTC4ei wallet
  software more difficult and which seriously complicates the design of smart
  contracts for MTC4ei.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each MTC4ei block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of mtc4eis they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of mtc4eis being spent is exactly
...
- **Increased security for multisig:** MTC4ei addresses (both P2PKH addresses
  ...
  believed to be provided by MTC4ei's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of MTC4ei's current ledger (called the UTXO set),
  ...
  note that MTC4ei Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  MTC4ei users to individually opt-in to almost any change in the MTC4ei
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and MTC4ei Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the MTC4ei
client to MTC4ei Core 0.13.2 to signal for segwit support. When a
...
Since MTC4ei Core 0.10.0, nodes have defaulted to only relaying and
...
- `mtc4ei-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `mtc4ei-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
